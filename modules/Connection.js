    ...
    onMessage(message) {
        ...
        if(this.player && isBinary && this.captcha.state == "ok") {
            ...
            if(len > 1 && message[len - 1] == String.fromCharCode(10)) {
                ...
                if(chat.length <= 96 || this.client.rank > permissions.user) {
                    if(chat[0] == "/") {
                        ...
                        if(this.client.chatc == "") {
                            server.players.sendToWorld(this.client.world, before + ": " + chat);
                            server.events.emit("chat", this.client, chat)
                        } else if(this.client.rank >= 2 && this.client.chatc !== "") {
                            server.players.sendToWorld(this.client.world, `<span style="color: ${this.client.chatc}">` + before + ": " + chat + "</span>");
                            server.events.emit("chat", this.client, chat)
                        };
                    }
                }
